...
- make one narrow code change in the selected current;
- run a red regression test, then the minimal green correction;
- observe a browser route and record console/network evidence;
- ask an independent critic to reject or accept the visible consequence;
- simplify or delete a rule that has not earned its presence;
- document a blocked or dormant current without manufacturing a tableau.
...
Every pulse and daily record must state:
