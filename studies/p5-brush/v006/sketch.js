...
function drawSeam(seam, index, active = false, pulse = 0) {
  ...
  context.save();
  context.globalAlpha = active ? .82 : .28 + Math.min(index, 5) * .035;
  const mouth = context.createRadialGradient(point.x, point.y, 0, point.x, point.y, radius * 1.9);
  mouth.addColorStop(0, active ? 'rgba(2, 3, 2, .97)' : 'rgba(4, 5, 4, .86)');
  mouth.addColorStop(.48, 'rgba(29, 20, 13, .48)');
  mouth.addColorStop(1, 'rgba(29, 20, 13, 0)');
  context.fillStyle = mouth;
  context.beginPath();
  context.ellipse(point.x, point.y, radius * 1.3, radius * .68, -.24 + index * .14, 0, Math.PI * 2);
  context.fill();

  context.globalAlpha = active ? .82 : .25;
...
function drawPool(point, size, color, alpha) {
  context.save();
  context.globalAlpha = alpha;
  const pool = context.createRadialGradient(point.x, point.y, 0, point.x, point.y, size * 1.8);
  pool.addColorStop(0, color);
  pool.addColorStop(.45, color.replace(')', ', .45)').replace('rgb', 'rgba'));
  pool.addColorStop(1, 'rgba(0, 0, 0, 0)');
  context.fillStyle = pool;
  context.beginPath();
  context.ellipse(point.x, point.y, size * 1.5, size * .58, -.18, 0, Math.PI * 2);
  context.fill();
  context.restore();
}
...
function drawStroke(stroke, index, memoryCount) {
  ...
  const returnShift = stroke.returnShift;
  context.save();
  ...
  for (let pointIndex = 1; pointIndex < stroke.points.length; pointIndex += 1) {
    ...
    const pressure = Math.max(previous.pressure, point.pressure);
    context.globalAlpha = .38 + stroke.opacity * .42 + pressure * .28;
    context.strokeStyle = pressure > .018 ? palette.wetLight : baseColor;
    context.lineWidth = stroke.weight * (1.7 + pressure * 5.4);
    context.beginPath();
  ...
  context.save();
  context.globalAlpha = .13 + Math.min(memoryCount, 5) * .028 + Math.min(Math.abs(returnShift), .16) * .12;
  context.strokeStyle = palette.chalk;
  context.lineWidth = stroke.weight * .38;
  context.setLineDash([.003, .011 + index * .001]);
  trace(stroke.points, (point) => point.y + point.return * .12);
  context.stroke();
  context.setLineDash([]);
  context.restore();
  ...
  for (let pointIndex = 3; pointIndex < stroke.points.length - 2; pointIndex += 4) {
    const point = stroke.points[pointIndex];
    if (point.pressure < .018) continue;
    const poolSize = .006 + point.pressure * .04;
    drawPool(point, poolSize, point.pressure > .06 ? 'rgb(240, 211, 160)' : 'rgb(201, 102, 71)', .12 + point.pressure * 1.4);
  }
}
...
function drawNotation(frame, state) {
  ...
  context.fillText(state === 'sequence' ? 'MATTER / RETURNING' : state.toUpperCase(), .035, .945);
...
function render(frame, progress = 1, state = 'sequence') {
  ...
  memoryReadout.textContent = `${frame.memory.length} return seams remembered`;
}
...
function makeSeam(point) {
  ...
  paused = true;
  stateReadout.textContent = 'The routes keep the bend and return elsewhere.';
...
function liftLatest() {
  ...
  paused = true;
  stateReadout.textContent = 'The latest seam is lifted; the field has been rebuilt.';
  render(interactionFrame, 1, 'seam-lifted');
}
...
function releaseSequence() {
  ...
  paused = false;
  stateReadout.textContent = 'The wet marks are carrying their delayed return.';
  renderCurrent();
}
...
removeControl?.addEventListener('click', () => makeSeam({ x: .62, y: .51 }));
