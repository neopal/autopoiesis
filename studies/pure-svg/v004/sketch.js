import { PRIMITIVE_BUDGET, STAGES, applyRefusal, buildFrame, buildTimeline, deleteRefusal } from './engine.mjs';
...
function render(frame, progress = 1, state = 'sequence') {
  const body = `${closedPath(frame.points)} ${frame.aperture.holePath}`;
  const aperture = frame.aperture.holePath;
  ...
  field.innerHTML = `
    ...
    <path class="animal-shadow" fill-rule="evenodd" d="${body}"/>
    <path class="draft" pathLength="1" d="${draft}"/>
    ${limbs}
    <path class="animal-glow" fill-rule="evenodd" d="${body}"/>
    <path class="animal" fill-rule="evenodd" pathLength="1" stroke-dasharray="1" stroke-dashoffset="${progressOffset}" d="${body}"/>
    <path class="aperture-rim" d="${aperture}"/>
...
function PRIMITIVE_LABEL(frame) {
  return `${PRIMITIVE_BUDGET} MARKS / ${frame.limbs.filter((limb) => limb.route === 'detour').length} DETOURS / ${(frame.aperture.radius * 100).toFixed(0)} VOID`;
}
