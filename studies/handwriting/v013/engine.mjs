export const MASTER_SEED = 0x57563133;
...
function normalizeCadence(entry, index = 0) {
  ...
  return {
    id: String(entry?.id ?? `cadence-${index}`),
    source: String(entry?.source ?? 'timeline-cadence'),
...
function phraseWindow(event, closestIndex) {
  ...
  const approachStart = Math.max(6, Math.min(POINT_COUNT - 31, anchor - 8));
  const phraseStart = approachStart + 8;
  const phraseEnd = phraseStart + 6;
  const relayStart = phraseEnd + 1;
  const relayEnd = Math.min(POINT_COUNT - 5, relayStart + 13);
  const settleEnd = Math.min(POINT_COUNT - 2, relayEnd + 7);
  return { approachStart, phraseStart, phraseEnd, relayStart, relayEnd, settleEnd };
}
...
/**
 * v013 changes handwriting's rule from a finite chorus to a cadence relay.
 * A local family shares one phrase, then temporarily inherits neighbouring
 * route cadence before settling with an altered exit. The memory array is an
 * explicit input so the whole field can be replayed exactly.
 */
export function makeRoutes(stage, memory = []) {
  const random = rng(MASTER_SEED + stage * 104729);
  const activeCadences = memory.map(normalizeCadence).slice(-MEMORY_WINDOW);
  ...
  return drafts.map((draft, routeIndex) => {
    ...
    for (const event of activeCadences) {
      ...
      const beforeRelay = points[window.relayStart].y;
      ...
      for (let step = window.approachStart; step < POINT_COUNT; step += 1) {
        if (step < window.phraseStart) {
          ...
          const settle = smooth((step - window.relayEnd) / (window.settleEnd - window.relayEnd));
          const inherited = partnerDraft[step].y + exitOffset;
          points[step].y = lerp(inherited, draftPoints[step].y + exitOffset, settle);
          points[step].x += 0.0048 * direction * influence.value * (1 - settle * 0.55);
        } else {
      ...
      const cadenceShift = Math.abs(points[window.relayEnd].y - draftPoints[window.relayEnd].y);
    ...
    return {
      ...
      band,
      cadenceHistory,
      relayed: cadenceHistory.length > 0,
...
function summarizeCadences(routes, memory) {
  return memory.map((event, memoryIndex) => {
    const affected = routes.flatMap((route) => route.cadenceHistory
    ...
    return {
      ...
      maxCadenceShift: affected.length ? Number(Math.max(...affected.map(({ record }) => record.cadenceShift)).toFixed(4)) : 0
    };
  });
}
...
export function buildFrame(targetStage, memory = []) {
  ...
  const stableMemory = memory.map(normalizeCadence).slice(-MEMORY_WINDOW);
  const routes = makeRoutes(stage, stableMemory);
  const stageCadence = makeStageCadence(stage);
  return {
    stage,
    routes,
    memory: stableMemory,
    cadences: summarizeCadences(routes, stableMemory),
    newCadences: stageCadence ? [stageCadence] : []
  };
}
...
export function buildTimeline(finalStage = STAGES - 1) {
  ...
  for (let stage = 0; stage <= Math.min(STAGES - 1, Math.max(0, Math.floor(finalStage))); stage += 1) {
    ...
    frames.push(frame);
    memory = [...memory, ...frame.newCadences].slice(-MEMORY_WINDOW);
  }
  return frames;
}

export function applyCadence(frame, position) {
  const x = Number(position?.x);
  const y = Number(position?.y);
  const safeX = clamp(Number.isFinite(x) ? x : 0.5, 0.07, 0.93);
  const safeY = clamp(Number.isFinite(y) ? y : 0.5, 0.105, 0.805);
  const event = normalizeCadence({
    id: `visitor-cadence-${frame.stage}-${frame.memory.length}`,
    source: 'visitor-cadence',
    stage: frame.stage,
    x: safeX,
    y: safeY,
    force: 1,
...
export function removeLatestCadence(frame) {
