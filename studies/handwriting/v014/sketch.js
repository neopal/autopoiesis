...
const liftButton = document.querySelector('#lift-return');
const releaseButton = document.querySelector('#release-sequence');
const placeButton = document.querySelector('#place-return');
...
function drawReturnWitness(route, record, index) {
  if (blind) return;
  const phraseStart = pointToCanvas(route.points[record.phraseStart]);
  const relayEnd = pointToCanvas(route.points[record.relayEnd]);
  const replyEnd = pointToCanvas(route.points[record.replyEnd]);
  const active = record.id === latestVisitorId || record.id === selectedReturnId;
  ctx.save();
  ctx.globalCompositeOperation = 'screen';
  ...
  ctx.lineTo(replyEnd.x, replyEnd.y);
  ctx.stroke();
  ctx.setLineDash([]);
  ...
  ctx.arc(replyEnd.x, replyEnd.y, active ? 7 : 3.2, 0, Math.PI * 2);
  ctx.stroke();
  ctx.restore();
}

function drawCounterphraseTrace(route, record, index) {
  if (blind) return;
  const active = record.id === latestVisitorId || record.id === selectedReturnId;
  ctx.save();
  ctx.globalCompositeOperation = 'screen';
...
function drawCurrentRoute(route, index) {
  const answered = route.counterphraseHistory.length > 0;
  ctx.save();
  ctx.lineCap = 'round';
  ctx.lineJoin = 'round';
  ctx.strokeStyle = `rgba(243, 237, 223, ${answered ? .9 : .24 + (index % 3) * .055})`;
  ctx.lineWidth = answered ? 1.04 + route.weight * .38 : .56 + route.weight * .18;
  drawSmoothPath(route.points);
  ctx.restore();
  route.counterphraseHistory.forEach((record, recordIndex) => {
    drawCounterphraseTrace(route, record, recordIndex);
    drawReturnWitness(route, record, recordIndex);
  });
}

function drawMemory() {
  if (blind) return;
  frameState.returns.forEach((event, index) => {
    if (!event.answered) return;
    const point = pointToCanvas(event.point);
    const active = event.id === selectedReturnId || event.id === latestVisitorId;
    ctx.save();
    ctx.globalCompositeOperation = 'screen';
...
function drawFrame(now) {
  ...
  const answered = frameState.routes.filter((route) => route.counterphraseHistory.length > 0).length;
  const maxShift = frameState.returns.reduce((max, event) => Math.max(max, event.maxCounterShift), 0);
  drawLabel(`COUNTERPHRASE / ${String(stage).padStart(2, '0')}`, width * .04, height * .07, colors.ink);
  drawLabel(latestVisitorId ? 'VISITOR RETURN / ANSWER CARRIED' : 'MEMORY ACTIVE / THE LINE ANSWERS', width * .96, height * .07, latestVisitorId ? colors.accent : colors.signal, 'right');
  drawLabel(`${answered} ROUTES IN ANSWER · ${frameState.memory.length} RETURNS REMEMBERED · MAX SHIFT ${maxShift.toFixed(3)}`, width * .04, height * .93, colors.ghost);
  if (interactive && !staticPreview) drawLabel('CLICK TO RETURN · TAB TO ACTIONS · SPACE TO RETURN', width * .96, height * .93, colors.ghost, 'right');
  ...
  if (!paused && !reduced && !staticPreview) {
    ...
    if (nextStage !== frameState.stage) {
      ...
      selectedReturnId = frameState.memory.at(-1)?.id ?? null;
      updateButtons();
    }
...
function placeAt(position) {
  ...
  frameState = applyReturn(frameState, position);
  latestVisitorId = frameState.memory.at(-1)?.id ?? null;
  selectedReturnId = latestVisitorId;
  paused = true;
  updateState('The route answered later; its counterphrase now changes the exit.');
  updateButtons();
  drawFrame(performance.now());
}
...
function liftLatest() {
  ...
  frameState = removeLatestReturn(frameState);
  latestVisitorId = frameState.memory.at(-1)?.id ?? null;
  selectedReturnId = latestVisitorId;
  paused = true;
  updateState('The latest answer lifted; the earlier sentence is exact again.');
  updateButtons();
  drawFrame(performance.now());
}
...
function saveStill() {
  ...
  link.download = 'mutine-handwriting-v014-counterphrase.png';
...
canvas.addEventListener('keydown', (event) => {
  ...
  if (event.key.toLowerCase() === 'p') {
    ...
    updateState(paused ? 'Timeline paused; the answer is held.' : 'Timeline moving; the next answer is latent.');
  }
...
window.__mutineHandwritingV014 = {
  getState: () => ({
    stage: frameState.stage,
    memory: frameState.memory.map((event) => ({ ...event })),
    answeredRoutes: frameState.routes.filter((route) => route.counterphraseHistory.length > 0).length,
    maxCounterShift: frameState.returns.reduce((max, event) => Math.max(max, event.maxCounterShift), 0),
    paused,
    interactive,
    blind
  }),
