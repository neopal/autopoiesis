import { STAGES, applyBridge, bridgeSignature, buildTimeline, deleteLatestBridge, layoutForViewport } from './engine.mjs';

const canvas = document.querySelector('#field');
const fieldWrap = document.querySelector('.field-wrap');
const stageReadout = document.querySelector('[data-stage]');
const memoryReadout = document.querySelector('[data-memory]');
const bridgeControl = document.querySelector('#bridge-control');
...
function stateSnapshot() {
  ...
  return {
    stage: frame.stage,
    stages: STAGES,
    memory: frame.scene.bridgeRecords.length,
    engineMemory: frame.memory.length,
    structuralMemory: frame.scene.bridgeRecords.length,
    approachPoints: frame.scene.route.filter((point) => point.phase === 'approach').length,
    risePoints: frame.scene.route.filter((point) => point.phase === 'rise').length,
    spanPoints: frame.scene.route.filter((point) => point.phase === 'span').length,
    dropPoints: frame.scene.route.filter((point) => point.phase === 'drop').length,
    reunitePoints: frame.scene.route.filter((point) => point.phase === 'reunite').length,
    interaction: interactionFrame?.interaction ?? 'sequence',
    bridgeSignature: bridgeSignature(frame)
  };
}

window.__mutineNaiveV012 = { getState: stateSnapshot };
canvas.dataset.witness = 'route-bridge';
if (staticPreview) {
  canvas.tabIndex = -1;
...
function drawGround(world, panel, draft) {
  ...
  stroke([world.ground.left, ...(world.ground.bridgeLine ?? []), world.ground.right], panel, '#2c292b', 4.3);
  stroke(world.ground.bridgeLine, panel, '#b6533e', 1.15);
}

function drawBridgeMarks(points, panel) {
  if (staticPreview) return;
  ctx.save();
  ctx.fillStyle = '#f8e6b9';
  ctx.strokeStyle = '#2c292b';
  ctx.lineWidth = 1.7;
  points.filter((point) => ['rise', 'span', 'drop'].includes(point.phase)).forEach((point) => {
    const mapped = mapPoint(panel, point);
    ctx.beginPath(); ctx.arc(mapped.x, mapped.y, point.phase === 'span' ? 4.5 : 3.4, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  });
  ctx.restore();
}

function drawBridgeSupports(points, panel) {
  if (staticPreview) return;
  const rise = points.find((point) => point.phase === 'rise');
  const drop = points.find((point) => point.phase === 'drop');
  const base = points.find((point) => point.phase === 'approach');
  if (!rise || !drop || !base) return;
  ctx.save();
  ctx.strokeStyle = 'rgba(44,41,43,.34)';
  ctx.lineWidth = 1.4;
  for (const support of [rise, drop]) {
    const top = mapPoint(panel, support);
    const bottom = mapPoint(panel, { x: support.x, y: base.y });
...
function drawHouse(world, panel, draft, progress) {
  ...
  if (!draft && world.house.bridgeLine.length) {
    const bridgeLine = [front.right, ...world.house.bridgeLine];
    drawBridgeSupports(world.house.bridgeLine, panel);
    stroke(bridgeLine, panel, '#2c292b', 5.5);
    stroke(bridgeLine, panel, '#b6533e', 1.25);
    drawBridgeMarks(world.house.bridgeLine, panel);
    stroke([world.house.bridgeLine.at(-1), world.house.roof.returnPeak], panel, '#2c292b', 2.2);
  }
  ctx.restore();
}
...
function drawPanel(frame, panel, kind, progress) {
  ...
  if (!draft && world.route.some((point) => point.phase === 'span')) {
    drawBridgeSupports(world.route.filter((point) => point.phase), panel);
    stroke(world.route.filter((point) => point.phase), panel, '#b6533e', 1.0);
    drawBridgeMarks(world.route.filter((point) => point.phase), panel);
  }
  ...
  if (!staticPreview) {
    ...
    ctx.textAlign = 'right'; ctx.fillStyle = '#4f7b80'; ctx.fillText(draft ? 'direct' : `${frame.scene.bridgeRecords.length} bridges kept`, b.x, b.y);
    ctx.restore();
  }
}
...
function render(frame, progress = 1, state = 'sequence') {
  ...
  if (!staticPreview) {
    ctx.save(); ctx.fillStyle = '#2c292b'; ctx.font = '10px Arial'; ctx.fillText(status, 46, viewHeight - 36); ctx.textAlign = 'right'; ctx.fillStyle = '#b6533e'; ctx.fillText(`${frame.primitiveCount} SHAPES / ${frame.scene.bridgeRecords.length} BRIDGES`, 954, viewHeight - 36); ctx.restore();
  }
  stageReadout.textContent = state === 'visitor-bridge' ? 'visitor bridge / paused' : state === 'bridge-lifted' ? 'latest bridge undone' : `stage ${String(frame.stage + 1).padStart(2, '0')} / ${STAGES}`;
  memoryReadout.textContent = `${frame.scene.bridgeRecords.length} bridge${frame.scene.bridgeRecords.length === 1 ? '' : 's'} retained`;
}
...
function makeBridge(point) {
  if (staticPreview) return;
  const base = interactionFrame?.interaction === 'visitor-bridge' ? interactionFrame : timeline[currentStage];
  interactionFrame = applyBridge(base, point);
  render(interactionFrame, 1, 'visitor-bridge');
}

function undoBridge() {
  if (staticPreview) return;
  const base = interactionFrame ?? timeline[currentStage];
  interactionFrame = deleteLatestBridge(base);
  render(interactionFrame, 1, 'bridge-lifted');
}
...
canvas.addEventListener('pointerdown', (event) => {
  ...
  makeBridge(pointerPoint(event));
});
canvas.addEventListener('keydown', (event) => {
  if (staticPreview || !['Enter', ' '].includes(event.key)) return;
  event.preventDefault();
  makeBridge({ x: 0.78, y: 0.34 });
});
bridgeControl.addEventListener('click', () => makeBridge({ x: 0.78, y: 0.34 }));
undoControl.addEventListener('click', undoBridge);
