import { STAGES, applyBypass, buildTimeline, bypassSignature, deleteLatestBypass, layoutForViewport } from './engine.mjs';

const canvas = document.querySelector('#field');
const fieldWrap = document.querySelector('.field-wrap');
const stageReadout = document.querySelector('[data-stage]');
const memoryReadout = document.querySelector('[data-memory]');
const bypassControl = document.querySelector('#bypass-control');
...
function stateSnapshot() {
  ...
  return {
    ...
    structuralMemory: frame.scene.bypassRecords.length,
    approachPoints: frame.scene.route.filter((point) => point.phase === 'approach').length,
    sidestepPoints: frame.scene.route.filter((point) => point.phase === 'sidestep').length,
    alongsidePoints: frame.scene.route.filter((point) => point.phase === 'alongside').length,
    returnPoints: frame.scene.route.filter((point) => point.phase === 'return').length,
    interaction: interactionFrame?.interaction ?? 'sequence',
    bypassSignature: bypassSignature(frame)
  };
}

window.__mutineNaiveV010 = { getState: stateSnapshot };
canvas.dataset.witness = 'route-bypass';
if (staticPreview) {
  canvas.tabIndex = -1;
...
function drawPaper() {
  ...
  gradient.addColorStop(0, '#f2dfb8');
  gradient.addColorStop(0.48, '#d5b47d');
  gradient.addColorStop(1, '#8b7273');
  ctx.fillStyle = gradient;
  ctx.fillRect(0, 0, 1000, viewHeight);
  const glow = ctx.createRadialGradient(170, 92, 5, 170, 92, 320);
  glow.addColorStop(0, 'rgba(255,245,192,.55)');
  glow.addColorStop(1, 'rgba(255,245,192,0)');
  ctx.fillStyle = glow;
  ctx.fillRect(0, 0, 1000, viewHeight);
  ctx.save();
  ctx.globalAlpha = 0.14;
  ctx.strokeStyle = '#4c7880';
  ctx.lineWidth = 1;
...
function drawSun(sun, panel) {
  ...
  ctx.save();
  ctx.strokeStyle = '#b54e3b';
  ctx.fillStyle = '#efa646';
  ctx.lineWidth = 3;
...
function drawTree(tree, panel, ground) {
  stroke([{ x: tree.x, y: tree.y + 0.05 }, { x: tree.x, y: ground }], panel, '#b54e3b', 7);
  const crown = [{ x: tree.x, y: tree.y - 0.11 }, { x: tree.x - 0.10, y: tree.y + 0.10 }, { x: tree.x + 0.075, y: tree.y + 0.07 }];
  fill(crown, panel, '#637b82');
  stroke(crown, panel, '#29262a', 2.8, true);
  const top = mapPoint(panel, { x: tree.x, y: tree.y + 0.05 });
  ctx.save(); ctx.fillStyle = '#b54e3b'; ctx.beginPath(); ctx.arc(top.x, top.y, 2.5, 0, Math.PI * 2); ctx.fill(); ctx.restore();
}

function drawGround(world, panel, draft) {
  if (draft) {
    stroke([{ x: 0.06, y: world.ground.y }, { x: 0.94, y: world.ground.y }], panel, '#637b82', 2.8);
    return;
  }
  stroke([world.ground.left, ...(world.ground.bypassLine ?? []), world.ground.right], panel, '#29262a', 4.1);
  stroke(world.ground.bypassLine, panel, '#b54e3b', 1.1);
}

function drawBypassMarks(points, panel) {
  ctx.save();
  ctx.fillStyle = '#f4dfb0';
  ctx.strokeStyle = '#29262a';
  ctx.lineWidth = 1.7;
  points.filter((point) => ['sidestep', 'return'].includes(point.phase)).forEach((point) => {
...
function drawHouse(world, panel, draft, progress) {
  ...
  fill(frontPolygon, panel, draft ? '#f2d4a7' : '#efad52');
  ctx.shadowColor = 'transparent'; ctx.shadowBlur = 0; ctx.shadowOffsetX = 0; ctx.shadowOffsetY = 0;
  fill(roof, panel, draft ? '#d17a5d' : '#b54e3b');
  stroke(frontPolygon, panel, draft ? '#b54e3b' : '#29262a', 3.2, true, draft ? [14, 12] : []);
  stroke(roof, panel, draft ? '#b54e3b' : '#29262a', 3.2, true, draft ? [14, 12] : []);
  const portal = [{ x: 0.48, y: 0.56 }, { x: 0.48, y: 0.69 }, { x: 0.52, y: 0.69 }, { x: 0.52, y: 0.56 }];
  fill(portal, panel, '#29262a');
  stroke(portal, panel, '#b54e3b', 2.2, true, draft ? [7, 7] : []);
  const window = [{ x: 0.523, y: 0.455 }, { x: 0.587, y: 0.455 }, { x: 0.587, y: 0.525 }, { x: 0.523, y: 0.525 }];
  fill(window, panel, '#4c7880');
  stroke(window, panel, '#29262a', 2, true);
  if (!draft && world.house.bypassLine.length) {
    stroke(world.house.bypassLine, panel, '#29262a', 5.4);
    stroke(world.house.bypassLine, panel, '#b54e3b', 1.25);
    if (!staticPreview) drawBypassMarks(world.house.bypassLine, panel);
    stroke([front.right, world.house.roof.returnPeak], panel, '#29262a', 2.2);
  }
  ctx.restore();
}
...
function drawPanel(frame, panel, kind, progress) {
  ...
  stroke(world.route, panel, draft ? '#b54e3b' : '#29262a', draft ? 4.4 : 5.5, false, draft ? [14, 13] : []);
  if (!draft && world.route.some((point) => point.phase === 'sidestep')) stroke(world.route.filter((point) => point.phase), panel, '#b54e3b', 1.0);
  drawHouse(world, panel, draft, progress);
  if (!staticPreview) {
    const a = mapPoint(panel, { x: 0.055, y: 0.075 });
    const b = mapPoint(panel, { x: 0.945, y: 0.075 });
    ctx.save();
    ctx.fillStyle = '#b54e3b'; ctx.font = '10px Arial'; ctx.letterSpacing = '1px'; ctx.fillText(label, a.x, a.y);
    ctx.textAlign = 'right'; ctx.fillStyle = '#4c7880'; ctx.fillText(draft ? 'direct' : `${frame.scene.bypassRecords.length} bypasses kept`, b.x, b.y);
    ctx.restore();
  }
}
...
function drawBridge(layout, frame) {
  ...
  ctx.strokeStyle = '#b54e3b'; ctx.lineCap = 'round'; ctx.lineWidth = 4.5; ctx.setLineDash([4, 12]);
  ctx.beginPath(); ctx.moveTo(from.x, from.y); ctx.bezierCurveTo(middle.x, from.y, middle.x, to.y, to.x, to.y); ctx.stroke();
  ctx.setLineDash([]); ctx.fillStyle = '#b54e3b';
...
function render(frame, progress = 1, state = 'sequence') {
  ...
  if (!staticPreview) {
    ctx.save(); ctx.fillStyle = '#29262a'; ctx.font = '10px Arial'; ctx.fillText(status, 46, viewHeight - 36); ctx.textAlign = 'right'; ctx.fillStyle = '#b54e3b'; ctx.fillText(`${frame.primitiveCount} SHAPES / ${frame.scene.bypassRecords.length} BYPASSES`, 954, viewHeight - 36); ctx.restore();
  }
  stageReadout.textContent = state === 'visitor-bypass' ? 'visitor bypass / paused' : state === 'bypass-lifted' ? 'latest bypass undone' : `stage ${String(frame.stage + 1).padStart(2, '0')} / ${STAGES}`;
  memoryReadout.textContent = `${frame.scene.bypassRecords.length} bypasses retained`;
}
...
function makeBypass(point) {
  if (staticPreview) return;
  const base = interactionFrame?.interaction === 'visitor-bypass' ? interactionFrame : timeline[currentStage];
  interactionFrame = applyBypass(base, point);
  render(interactionFrame, 1, 'visitor-bypass');
}

function undoBypass() {
  if (staticPreview) return;
  const base = interactionFrame ?? timeline[currentStage];
  interactionFrame = deleteLatestBypass(base);
  render(interactionFrame, 1, 'bypass-lifted');
}
...
canvas.addEventListener('pointerdown', (event) => {
  ...
  makeBypass(pointerPoint(event));
});
canvas.addEventListener('keydown', (event) => {
  if (staticPreview || !['Enter', ' '].includes(event.key)) return;
  event.preventDefault();
  makeBypass({ x: 0.78, y: 0.34 });
});
bypassControl.addEventListener('click', () => makeBypass({ x: 0.78, y: 0.34 }));
undoControl.addEventListener('click', undoBypass);
