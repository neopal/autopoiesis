...
const canonical = `${base}/works/webgpu-2026-09-09/`;
const raw = `${base}/studies/webgpu/v005/?preview=1&interaction=1&blind=1`;
...
for (const reduced of [false, true]) {
  for (const [width, height] of targets) {
    ...
    const frame = page.frames().find((candidate) => candidate.url().includes('/studies/webgpu/v005/'));
    ...
    if (!outer.tableauFirst || outer.workId !== 'webgpu-2026-09-09') failures.push(`canonical shell ${width}x${height}`);
...
if (afterPointer.memory !== '1 index' || afterPointer.stage !== 'visitor index / paused') failures.push('pointer index');
if (afterPointerLift.memory !== '0 indexes' || afterPointerLift.signature !== before.signature) failures.push('pointer lift restoration');
if (afterKeyboard.memory !== '1 index' || afterKeyboard.stage !== 'visitor index / paused') failures.push('keyboard index');
if (afterKeyboardLift.memory !== '0 indexes') failures.push('keyboard lift');
if (afterRelease.memory !== '0 indexes') failures.push('release census');
...
await staticPage.goto(`${base}/studies/webgpu/v005/?preview=1&static=1&blind=1`, { waitUntil: 'networkidle' });
...
const staticScreenshot = 'mutine-v005-static-blind-390x844.png';
await staticPage.screenshot({ path: join(outputDir, staticScreenshot), fullPage: false });
if (staticPreview.innerWidth !== 390 || staticPreview.clientWidth !== 390 || staticPreview.scrollWidth > 390 || !staticPreview.preview || !staticPreview.static || !staticPreview.canvas || staticPreview.controlsVisible) failures.push('static preview');
...
await directRawPage.goto(`${base}/studies/webgpu/v005/`, { waitUntil: 'networkidle' });
const directRawRedirect = directRawPage.url();
await directRawPage.close();
if (!directRawRedirect.endsWith('/works/webgpu-2026-09-09/')) failures.push('raw bridge redirect');

await browser.close();
const result = {
  workId: 'webgpu-2026-09-09',
  study: '/studies/webgpu/v005/',
  canonicalRoute: '/works/webgpu-2026-09-09/',
