...
async function frameFor(page) {
  ...
  const frame = page.frames().find((candidate) => candidate.url().includes('/studies/self-portrait/v007/'));
...
async function runMatrix(browser) {
  ...
  for (const [width, height] of viewports) {
    for (const reducedMotion of [false, true]) {
      ...
      try {
        await page.goto(`${ROOT}/works/portrait-2026-09-11/`, { waitUntil: 'networkidle' });
        const frame = await frameFor(page);
        await page.waitForTimeout(reducedMotion ? 120 : 180);
        const state = await frame.evaluate(() => window.__mutinePortraitV007?.getState());
...
async function runRawInteraction(browser) {
  ...
  try {
    await page.goto(`${ROOT}/studies/self-portrait/v007/?preview=1&interaction=1`, { waitUntil: 'networkidle' });
    const frame = page.frames().find((candidate) => candidate.url().includes('/studies/self-portrait/v007/'));
    if (!frame) throw new Error('raw interactive v007 frame did not attach');
    const canvas = frame.locator('#field');
    await canvas.waitFor({ state: 'visible' });
    const initial = await frame.evaluate(() => window.__mutinePortraitV007?.getState());
    await canvas.click({ position: { x: 190, y: 320 } });
    const pointer = await frame.evaluate(() => window.__mutinePortraitV007?.getState());
    await canvas.focus();
    await canvas.press('Enter');
    const keyboard = await frame.evaluate(() => window.__mutinePortraitV007?.getState());
    await frame.locator('#undo-control').click();
    const lifted = await frame.evaluate(() => window.__mutinePortraitV007?.getState());
    await frame.locator('#release-control').click();
    const released = await frame.evaluate(() => window.__mutinePortraitV007?.getState());
...
async function runRawBlind(browser) {
  ...
  try {
    await page.goto(`${ROOT}/studies/self-portrait/v007/?preview=1&static=1`, { waitUntil: 'networkidle' });
    const frame = page.frames().find((candidate) => candidate.url().includes('/studies/self-portrait/v007/'));
...
try {
  ...
  const result = {
    ...
    route: `${ROOT}/works/portrait-2026-09-11/`,
    rawPreview: `${ROOT}/studies/self-portrait/v007/?preview=1&interaction=1`,
    matrix,
    interaction,
    blind
  };
