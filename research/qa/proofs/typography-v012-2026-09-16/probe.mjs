...
const canonicalPath = '/works/typography-2026-09-16/';
const rawPath = '/studies/handwriting/v012/?preview=1&interaction=1';
const viewports = [
...
async function openCanonical(browser, viewport, reducedMotion) {
  ...
  const frame = page.frames().find((candidate) => candidate.url().includes('/studies/handwriting/v012/'));
  assert(frame, 'embedded v012 frame did not load');
  await frame.waitForFunction(() => Boolean(window.__mutineHandwritingV012));
  ...
  const tableau = await frame.evaluate(() => ({
    ...
    state: window.__mutineHandwritingV012.getState(),
  ...
  if (reducedMotion) assert(tableau.state.stage === 16 && tableau.state.memory.length === 6 && tableau.state.paused, 'reduced motion did not settle the final chorus frame');
...
async function openInteraction(browser) {
  ...
  await page.waitForFunction(() => Boolean(window.__mutineHandwritingV012));
  ...
  assert(initial.controls.length === 3 && initial.controls.every((control) => control.height >= 44) && initial.controls.find((control) => control.id === 'place-chorus')?.disabled === false && initial.controls.find((control) => control.id === 'release-sequence')?.disabled === false && initial.controls.find((control) => control.id === 'lift-chorus')?.disabled === true, 'raw controls below 44px or wrong initial disabled state');
  ...
  const afterPointer = await page.evaluate(() => ({ state: window.__mutineHandwritingV012.getState(), signature: document.querySelector('#piece').toDataURL() }));
  assert(afterPointer.state.memory.length === 1 && afterPointer.state.paused && afterPointer.state.chorusedRoutes >= 3 && afterPointer.signature !== initial.signature, 'pointer did not change the structural field');
  await canvas.focus();
  await page.keyboard.press('Enter');
  const afterKeyboard = await page.evaluate(() => ({ state: window.__mutineHandwritingV012.getState() }));
  assert(afterKeyboard.state.memory.length === 2 && afterKeyboard.state.chorusedRoutes >= 3, 'keyboard did not add a second chorus');
  await page.locator('#lift-chorus').click();
  const afterLift = await page.evaluate(() => ({ state: window.__mutineHandwritingV012.getState(), signature: document.querySelector('#piece').toDataURL() }));
  assert(afterLift.state.memory.length === 1, 'lift did not remove only the latest chorus');
  ...
  const afterRelease = await page.evaluate(() => ({ state: window.__mutineHandwritingV012.getState() }));
...
async function openStaticBlind(browser) {
  ...
  const path = '/studies/handwriting/v012/?preview=1&static=1&blind=1';
...
async function openJournal(browser) {
  ...
  const journal = await page.evaluate(() => {
    const entry = document.querySelector('#journal-typography-2026-09-16');
    return {
      innerWidth,
      clientWidth: document.documentElement.clientWidth,
      scrollWidth: document.documentElement.scrollWidth,
      anchorCount: document.querySelectorAll('#journal-typography-2026-09-16').length,
      titlePresent: document.body.textContent.includes('The sentence enters a chorus.'),
      href: entry?.querySelector('h3 a')?.getAttribute('href') ?? null
    };
  });
  assert(journal.anchorCount === 1 && journal.titlePresent && journal.href === '/works/typography-2026-09-16/#journal', 'Journal record did not render');
...
async function checkRedirect(browser) {
  ...
  const page = await context.newPage();
  const response = await page.goto(`${base}/studies/handwriting/v012/`, { waitUntil: 'domcontentloaded' });
  results.redirect = { responseStatus: response?.status() ?? null, url: page.url() };
  assert(page.url().includes('/works/typography-2026-09-16/'), 'direct raw tableau did not redirect to canonical work');
  await context.close();
}
...
try {
  ...
  const faviconResponse = await fetch(`${base}/studio/favicon.svg?qa=v012-favicon`);
