...
const proofDir = 'research/qa/proofs/naive-art-v010-2026-09-14';
...
async function readStudy(frame) {
  return frame.evaluate(() => {
    ...
    const state = window.__mutineNaiveV010?.getState?.();
...
for (const reduced of [false, true]) {
  ...
  for (const [width, height] of viewports) {
    ...
    await page.goto(`${ROOT}/works/naive-2026-09-14/`, { waitUntil: 'networkidle' });
...

{
  ...
  await page.goto(`${ROOT}/studies/naive-art/v010/?preview=1&interaction=1`, { waitUntil: 'networkidle' });
...

{
  ...
  const events = observe(page);
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto(`${ROOT}/studies/naive-art/v010/?preview=1&static=1&blind=1`, { waitUntil: 'networkidle' });
