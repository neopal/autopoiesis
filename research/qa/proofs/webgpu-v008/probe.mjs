...
const canonical = `${base}/works/webgpu-2026-09-12/`;
const raw = `${base}/studies/webgpu/v008/?preview=1&interaction=1&blind=1`;
...
for (const reduced of [false, true]) {
  for (const [width, height] of targets) {
    ...
    const frame = page.frames().find((candidate) => candidate.url().includes('/studies/webgpu/v008/'));
    if (!frame) throw new Error(`v008 iframe missing at ${width}x${height}`);
    ...
    if (!outer.tableauFirst || outer.workId !== 'webgpu-2026-09-12') failures.push(`canonical shell ${width}x${height}`);
...
if (afterPointer.memory !== '1 echo' || afterPointer.stage !== 'visitor echo / paused') failures.push('pointer echo');
if (afterPointerLift.memory !== '0 echoes' || afterPointerLift.signature !== before.signature) failures.push('pointer lift restoration');
if (afterKeyboard.memory !== '1 echo' || afterKeyboard.stage !== 'visitor echo / paused') failures.push('keyboard echo');
if (afterKeyboardLift.memory !== '0 echoes') failures.push('keyboard lift');
if (afterRelease.memory !== '0 echoes') failures.push('release echo');
...
await staticPage.goto(`${base}/studies/webgpu/v008/?preview=1&static=1&blind=1`, { waitUntil: 'networkidle' });
...
await directRawPage.goto(`${base}/studies/webgpu/v008/`, { waitUntil: 'networkidle' });
const directRawRedirect = directRawPage.url();
await directRawPage.close();
if (!directRawRedirect.endsWith('/works/webgpu-2026-09-12/')) failures.push('raw bridge redirect');

await browser.close();
const result = {
  workId: 'webgpu-2026-09-12',
  study: '/studies/webgpu/v008/',
  canonicalRoute: '/works/webgpu-2026-09-12/',
