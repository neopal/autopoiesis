...
const PROOF_DIR = new URL('./', import.meta.url).pathname.replace(/^\//, '').replaceAll('/', '\\');
const viewports = [
...
const canonicalPath = '/works/webgpu-2026-09-16/';
const rawPath = '/studies/webgpu/v011/?preview=1&interaction=1&blind=1';
const blindPath = '/studies/webgpu/v011/?preview=1&interaction=1&blind=1&static=1';
...
const viewportEvidence = async (browser, viewport, reduced) => {
  ...
  await page.waitForTimeout(reduced ? 40 : 25);
  ...
  await page.screenshot({ path: `${PROOF_DIR}${process.env.MUTINE_FILE_PREFIX ?? ''}canonical-${viewport.name}-${reduced ? 'reduced' : 'normal'}.png`, fullPage: false });
  await context.close();
...
const rawInteractionEvidence = async (browser) => {
  ...
  await page.screenshot({ path: `${PROOF_DIR}${process.env.MUTINE_FILE_PREFIX ?? ''}raw-interaction-390x844.png`, fullPage: false });
  await context.close();
  return {
...
const blindEvidence = async (browser) => {
  ...
  await page.screenshot({ path: `${PROOF_DIR}${process.env.MUTINE_FILE_PREFIX ?? ''}static-blind-390x844.png`, fullPage: false });
  await context.close();
...
const routeEvidence = async (browser, path, selector, screenshotName) => {
  ...
  await page.screenshot({ path: `${PROOF_DIR}${screenshotName}`, fullPage: false });
  await context.close();
...
try {
  ...
  const filePrefix = process.env.MUTINE_FILE_PREFIX ?? '';
  const journal = await routeEvidence(browser, '/journal/', '#journal-webgpu-2026-09-16', `${filePrefix}journal-390x844.png`);
  const current = await routeEvidence(browser, '/currents/webgpu/', '[data-catalog-current-header="webgpu"]', `${filePrefix}current-webgpu-390x844.png`);
  const results = { generatedAt: new Date().toISOString(), matrix, raw, blind, journal, current };
  await writeFile(new URL(`./${process.env.MUTINE_RESULT_FILENAME ?? 'results.json'}`, import.meta.url), JSON.stringify(results, null, 2) + '\n');
  console.log(JSON.stringify({ matrixRuns: matrix.length, raw, blind, journal, current, resultsPath: new URL(`./${process.env.MUTINE_RESULT_FILENAME ?? 'results.json'}`, import.meta.url).pathname }, null, 2));
} finally {
  await browser.close();
}
