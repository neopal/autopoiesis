...
const PROOF_DIR = path.resolve('research/qa/proofs/svg-v010-2026-09-13');
...
function issueSnapshot(page, bucket) {
  return { console: bucket.console, pageErrors: bucket.pageErrors, requestFailures: bucket.requestFailures, httpErrors: bucket.httpErrors, url: page.url() };
}
...
async function canonicalRun(browser, width, height, reducedMotion) {
  ...
  const response = await page.goto(`${BASE}/works/svg-2026-09-13/`, { waitUntil: 'networkidle' });
  ...
  const outer = await page.evaluate(() => {
    const overflowElements = [...document.querySelectorAll('*')].map((element) => {
      ...
      return { tag: element.tagName.toLowerCase(), className: typeof element.className === 'string' ? element.className : '', right: rect.right, width: rect.width };
  ...
  const study = page.frames().find((frame) => frame.url().includes('/studies/pure-svg/v010/'));
  ...
  check(outer.title === 'The animal keeps a counterweight.', `canonical title ${width}x${height}`, outer.title);
...
async function rawInteractionRun(browser) {
  ...
  const issues = { console: [], pageErrors: [], requestFailures: [], httpErrors: [] };
  attachIssues(page, issues);
  const response = await page.goto(`${BASE}/studies/pure-svg/v010/?preview=1&interaction=1`, { waitUntil: 'networkidle' });
  ...
  const afterPointer = await page.evaluate(() => ({ memory: document.querySelector('#field')?.dataset.memory, signature: window.__mutinePureSvgV010?.getFrameSignature() }));
  check(afterPointer.memory === '1', 'pointer adds one counterweight', afterPointer);

  await field.focus();
  await page.keyboard.press('Enter');
  await page.waitForFunction(() => document.querySelector('#field')?.dataset.memory === '2');
  const afterKeyboard = await page.evaluate(() => ({ memory: document.querySelector('#field')?.dataset.memory, signature: window.__mutinePureSvgV010?.getFrameSignature() }));
  check(afterKeyboard.memory === '2', 'keyboard adds second counterweight', afterKeyboard);
  ...
  await page.waitForFunction(() => document.querySelector('#field')?.dataset.memory === '1' && document.querySelector('#field')?.dataset.weightState === 'unweighted');
  const afterLift = await page.evaluate(() => ({ memory: document.querySelector('#field')?.dataset.memory, state: document.querySelector('#field')?.dataset.weightState, signature: window.__mutinePureSvgV010?.getFrameSignature() }));
  check(afterLift.memory === '1' && afterLift.state === 'unweighted', 'lift removes latest counterweight', afterLift);
  ...
  const afterRelease = await page.evaluate(() => ({ memory: document.querySelector('#field')?.dataset.memory, state: document.querySelector('#field')?.dataset.weightState }));
  ...
  interaction.sequence = { afterPointer, afterKeyboard, afterLift, afterRelease, postInteraction, issues: issuesResult, screenshots: [initialCapture, afterCapture] };
  await context.close();
}
...
async function staticBlindRun(browser) {
  ...
  const issues = { console: [], pageErrors: [], requestFailures: [], httpErrors: [] };
  attachIssues(page, issues);
  const response = await page.goto(`${BASE}/studies/pure-svg/v010/?preview=1&static=1&blind=1`, { waitUntil: 'networkidle' });
  await page.locator('#field').waitFor();
  const state = await page.evaluate(() => {
    const selectors = ['.field-readout', '.field-controls', '.artwork-label', '.counterweight-witness', '.counterweight-thread'];
    ...
    return { innerWidth, clientWidth: document.documentElement.clientWidth, scrollWidth: document.documentElement.scrollWidth, svg: { width: svg?.width, height: svg?.height }, hidden, memory: document.querySelector('#field')?.dataset.memory };
  });
  ...
  check(state.memory === '4', 'static blind renders settled memory', state.memory);
...
const result = { passed: failures.length === 0, failureCount: failures.length, failures, runs, interaction, generatedAt: new Date().toISOString() };
