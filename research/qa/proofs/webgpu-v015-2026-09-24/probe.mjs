...
const mime = { '.html': 'text/html; charset=utf-8', '.js': 'text/javascript; charset=utf-8', '.mjs': 'text/javascript; charset=utf-8', '.css': 'text/css; charset=utf-8', '.json': 'application/json; charset=utf-8', '.svg': 'image/svg+xml', '.ico': 'image/x-icon' };
let baseUrl;
...
async function runMatrix(browser, path, kind, viewport, reduced) {
  ...
  const file = join(proofDir, `${kind}-${viewport.name}-${reduced ? 'reduced' : 'normal'}.png`);
  await page.screenshot({ path: file, fullPage: false });
  await context.close();
  return { kind, viewport: viewport.name, motion: reduced ? 'reduced' : 'normal', httpStatus: response?.status() ?? null, snapshot, outer, tableau, diagnostics: diag, screenshot: file };
}
...
async function runInteraction(browser) {
  ...
  const outer = await outerSnapshot(page);
  const file = join(proofDir, 'raw-interaction-390x844.png');
  await page.screenshot({ path: file, fullPage: false });
  await context.close();
  return {
    ...
    controls,
    outer,
    changedOnWitness: beforeImage !== witnessedImage,
    changedOnKeyboard: witnessedImage !== keyboardImage,
    liftRestoredWitness: liftedImage === witnessedImage,
...
async function runBlind(browser) {
  ...
  const file = join(proofDir, 'static-blind-390x844.png');
  await page.screenshot({ path: file, fullPage: false });
  await context.close();
  return { httpStatus: response?.status() ?? null, evidence, diagnostics: diag, screenshot: file };
}
...
async function runRoute(browser, path, selector, file) {
  ...
  const screenshot = join(proofDir, file);
...
try {
  ...
  const journal = await runRoute(browser, '/journal/', '#journal-webgpu-2026-09-24', 'journal-390x844.png');
  ...
  await writeFile(join(proofDir, 'results.json'), `${JSON.stringify(results, null, 2)}\n`);
  const matrixOverflow = [...matrix, ...rawMatrix].filter((run) => run.outer.scrollWidth > run.outer.innerWidth);
  const allIssues = [...matrix, ...rawMatrix, interaction, blind, journal, current].flatMap(issues);
  const summary = {
    canonicalRuns: matrix.length,
    rawRuns: rawMatrix.length,
    viewportOverflow: matrixOverflow.length,
