...
const raw = `${base}/studies/webgpu/v007/?preview=1&interaction=1&blind=1`;
...
for (const reduced of [false, true]) {
  for (const [width, height] of targets) {
    ...
    if (!frame) throw new Error(`v007 iframe missing at ${width}x${height}`);
    ...
    if (!outer.tableauFirst || outer.workId !== 'webgpu-2026-09-11') failures.push(`canonical shell ${width}x${height}`);
...
if (afterPointer.memory !== '1 absence' || afterPointer.stage !== 'visitor absence / paused') failures.push('pointer absence');
if (afterPointerLift.memory !== '0 absences' || afterPointerLift.signature !== before.signature) failures.push('pointer lift restoration');
if (afterKeyboard.memory !== '1 absence' || afterKeyboard.stage !== 'visitor absence / paused') failures.push('keyboard absence');
if (afterKeyboardLift.memory !== '0 absences') failures.push('keyboard lift');
if (afterRelease.memory !== '0 absences') failures.push('release census');
...
await staticPage.goto(`${base}/studies/webgpu/v007/?preview=1&static=1&blind=1`, { waitUntil: 'networkidle' });
...
await directRawPage.goto(`${base}/studies/webgpu/v007/`, { waitUntil: 'networkidle' });
const directRawRedirect = directRawPage.url();
await directRawPage.close();
if (!directRawRedirect.endsWith('/works/webgpu-2026-09-11/')) failures.push('raw bridge redirect');

await browser.close();
const result = {
  workId: 'webgpu-2026-09-11',
  study: '/studies/webgpu/v007/',
  canonicalRoute: '/works/webgpu-2026-09-11/',
