...
async function waitForTableau(page) {
  ...
  await page.waitForFunction(() => {
    ...
    const canvas = doc?.querySelector('#field');
    const button = doc?.querySelector('#make-basin');
    const canvasRect = canvas?.getBoundingClientRect();
    return Boolean(canvasRect && canvasRect.width > 250 && canvasRect.height > 250 && button?.getBoundingClientRect().height >= 44);
  }, { timeout: 10000 });
