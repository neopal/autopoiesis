...
const OUT = 'research/qa/proofs/naive-art-v007-2026-09-11/production';
...
async function frameFor(page) {
  ...
  if (!frame) throw new Error('production v007 tableau iframe did not attach');
...
async function runMatrix(browser) {
  ...
  for (const [width, height] of viewports) {
    for (const reducedMotion of [false, true]) {
      ...
      try {
        const response = await page.goto(`${ROOT}/works/naive-2026-09-11/`, { waitUntil: 'networkidle' });
        ...
        results.push({ width, height, reducedMotion, status: response?.status(), state, layout, capture: file, ...issuesFor(events) });
      } finally {
        await page.close();
      }
    }
...
async function runJournal(browser) {
  ...
  try {
    ...
    const entry = page.locator('#journal-naive-2026-09-11');
    const journal = await entry.evaluate((node) => ({
      count: 1,
    ...
    return { status: response?.status(), journal, capture: 'journal-390x844.png', ...issuesFor(events) };
  } finally {
    await page.close();
  }
}
...
async function runRawInteraction(browser) {
  ...
  try {
    const response = await page.goto(`${ROOT}/studies/naive-art/v007/?preview=1&interaction=1`, { waitUntil: 'networkidle' });
    const frame = page.frames().find((candidate) => candidate.url().includes('/studies/naive-art/v007/'));
    if (!frame) throw new Error('production raw interactive v007 frame did not attach');
    ...
    return { status: response?.status(), initial, pointer, keyboard, lifted, released, controls, layout: await geometry(page, frame), capture: 'raw-interaction-390x844.png', ...issuesFor(events) };
  } finally {
    await page.close();
  }
}
...
async function runRawBlind(browser) {
  const { page, events } = await newPage(browser, { width: 390, height: 844 }, true);
  try {
    const response = await page.goto(`${ROOT}/studies/naive-art/v007/?preview=1&static=1`, { waitUntil: 'networkidle' });
    const frame = page.frames().find((candidate) => candidate.url().includes('/studies/naive-art/v007/'));
    if (!frame) throw new Error('production raw blind v007 frame did not attach');
    await frame.locator('#field').waitFor({ state: 'visible' });
    const state = await frame.evaluate(() => window.__mutineNaiveV007?.getState());
    ...
    return { status: response?.status(), state, hidden, layout: await geometry(page, frame), capture: 'raw-blind-390x844.png', ...issuesFor(events) };
  } finally {
    await page.close();
  }
}
...
try {
  ...
  const result = { generatedAt: new Date().toISOString(), route: `${ROOT}/works/naive-2026-09-11/`, rawPreview: `${ROOT}/studies/naive-art/v007/?preview=1&interaction=1`, matrix, journal, interaction, blind };
  await writeFile(`${OUT}/results.json`, `${JSON.stringify(result, null, 2)}\n`);
  process.stdout.write(`${JSON.stringify({ matrix: matrix.length, journal, interaction, blind }, null, 2)}\n`);
} finally {
  await browser.close();
}
