...
const canonicalPath = '/works/typography-2026-09-17/';
const rawPath = '/studies/handwriting/v013/?preview=1&interaction=1';
const viewports = [
...
async function openCanonical(browser, viewport, reducedMotion) {
  ...
  const frame = page.frames().find((candidate) => candidate.url().includes('/studies/handwriting/v013/'));
  assert(frame, 'embedded v013 frame did not load');
  await frame.waitForFunction(() => Boolean(window.__mutineHandwritingV013));
  ...
  const tableau = await frame.evaluate(() => ({
    ...
    state: window.__mutineHandwritingV013.getState(),
...
async function openInteraction(browser) {
  ...
  await page.waitForFunction(() => Boolean(window.__mutineHandwritingV013));
  ...
  assert(initial.controls.length === 3 && initial.controls.every((control) => control.height >= 44) && initial.controls.find((control) => control.id === 'place-cadence')?.disabled === false && initial.controls.find((control) => control.id === 'release-sequence')?.disabled === false && initial.controls.find((control) => control.id === 'lift-cadence')?.disabled === true, 'raw controls below 44px or wrong initial disabled state');
  ...
  const afterPointer = await page.evaluate(() => ({ state: window.__mutineHandwritingV013.getState(), signature: document.querySelector('#piece').toDataURL() }));
  assert(afterPointer.state.memory.length === 1 && afterPointer.state.paused && afterPointer.state.relayedRoutes >= 3 && afterPointer.signature !== initial.signature, 'pointer did not change the structural field');
  await canvas.focus();
  await page.keyboard.press('Enter');
  const afterKeyboard = await page.evaluate(() => ({ state: window.__mutineHandwritingV013.getState() }));
  assert(afterKeyboard.state.memory.length === 2 && afterKeyboard.state.relayedRoutes >= 3, 'keyboard did not add a second cadence');
  await page.locator('#lift-cadence').click();
  const afterLift = await page.evaluate(() => ({ state: window.__mutineHandwritingV013.getState(), signature: document.querySelector('#piece').toDataURL() }));
  ...
  const afterRelease = await page.evaluate(() => ({ state: window.__mutineHandwritingV013.getState() }));
...
async function openStaticBlind(browser) {
  ...
  const path = '/studies/handwriting/v013/?preview=1&static=1&blind=1';
...
async function openJournal(browser) {
  ...
  const journal = await page.evaluate(() => {
    const entry = document.querySelector('#journal-typography-2026-09-17');
    return {
      innerWidth,
      clientWidth: document.documentElement.clientWidth,
      scrollWidth: document.documentElement.scrollWidth,
      anchorCount: document.querySelectorAll('#journal-typography-2026-09-17').length,
      titlePresent: document.body.textContent.includes('The sentence passes its cadence.'),
      href: entry?.querySelector('h3 a')?.getAttribute('href') ?? null
    };
  });
  assert(journal.anchorCount === 1 && journal.titlePresent && journal.href === '/works/typography-2026-09-17/#journal', 'Journal record did not render');
...
async function checkRedirect(browser) {
  ...
  const page = await context.newPage();
  const response = await page.goto(`${base}/studies/handwriting/v013/`, { waitUntil: 'domcontentloaded' });
  results.redirect = { responseStatus: response?.status() ?? null, url: page.url() };
  assert(page.url().includes('/works/typography-2026-09-17/'), 'direct raw tableau did not redirect to canonical work');
  await context.close();
}
...
try {
  ...
  const faviconResponse = await fetch(`${base}/studio/favicon.svg?qa=v013-favicon`);
