...
const proofDir = resolve('research/qa/proofs/naive-art-v016-2026-09-23/production');
...
const rawPath = '/studies/naive-art/v016/?preview=1&interaction=1';
const blindPath = '/studies/naive-art/v016/?preview=1&static=1&blind=1';
const canonicalPath = '/works/naive-2026-09-23/';
...
async function tableauState(page) {
  const frame = page.frames().find((candidate) => candidate.url().includes('/studies/naive-art/v016/'));
  if (!frame) return null;
  return frame.evaluate(() => ({
    state: window.__mutineNaiveV016?.getState() ?? null,
...
async function runInteraction(browser) {
  ...
  const initial = await page.evaluate(() => window.__mutineNaiveV016.getState());
  ...
  const present = await page.evaluate(() => window.__mutineNaiveV016.getState());
  await page.mouse.move(box.x + box.width + 18, box.y + box.height * 0.24);
  const departure = await page.evaluate(() => window.__mutineNaiveV016.getState());
  await field.focus();
  await page.keyboard.press('Enter');
  const keyboard = await page.evaluate(() => window.__mutineNaiveV016.getState());
  await page.keyboard.press('Delete');
  const lifted = await page.evaluate(() => window.__mutineNaiveV016.getState());
  ...
  const released = await page.evaluate(() => window.__mutineNaiveV016.getState());
  const outer = await outerSnapshot(page);
  ...
  return {
    ...
    outer,
    diagnostics: diag,
    presenceRefused: present.departureSignature === initial.departureSignature && present.memory === initial.memory,
    exactLiftRestored: lifted.departureSignature === departure.departureSignature,
...
async function runBlind(browser) {
  ...
  const evidence = await page.evaluate(() => ({
    state: window.__mutineNaiveV016.getState(),
    outer: { innerWidth: innerWidth, clientWidth: document.documentElement.clientWidth, scrollWidth: document.documentElement.scrollWidth },
...
try {
  ...
  const journal = await runRoute(browser, '/journal/', '#journal-naive-2026-09-23', 'journal-390x844.png');
  ...
  if (issueCount || overflowCount || tableauFirstCount !== matrix.length || !interaction.presenceRefused || !interaction.exactLiftRestored || !interaction.releaseCleared) process.exitCode = 1;
} finally {
  await browser.close();
}
