...
const proofDir = 'research/qa/proofs/brush-v011-2026-09-13';
const viewports = [
  [320, 568],
  [390, 844],
  [768, 1024],
...
async function readOuter(page) {
  return page.evaluate(() => {
    ...
    }).filter((item) => (item.left < -1 || item.right > innerWidth + 1) && !item.element.closest('.work-timeline-bar')).map(({ element, name, left, right }) => ({ element: name, left, right })).slice(0, 12);
    const intentionalScroll = [...document.querySelectorAll('.work-timeline-bar')].map((element) => ({ scrollWidth: element.scrollWidth, clientWidth: element.clientWidth }));
...
async function findStudyFrame(page) {
  await page.waitForFunction(() => [...document.querySelectorAll('iframe')].some((iframe) => iframe.src.includes('/studies/p5-brush/v011/')));
  await page.waitForTimeout(80);
  const frame = page.frames().find((candidate) => candidate.url().includes('/studies/p5-brush/v011/'));
...
for (const reduced of [false, true]) {
  ...
  for (const [width, height] of viewports) {
    ...
    await page.goto(`${ROOT}/works/brush-2026-09-13/`, { waitUntil: 'networkidle' });
...

{
  ...
  await page.goto(`${ROOT}/studies/p5-brush/v011/?preview=1&interaction=1`, { waitUntil: 'networkidle' });
  ...
  results.local.raw = {
    ...
    secondLiftReturnedToZero: afterSecondLift.memory === '0 siphons remembered',
    issues: events
  };
  await page.close();
  await context.close();
...

{
  ...
  const events = observe(page);
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto(`${ROOT}/studies/p5-brush/v011/?preview=1&static=1&blind=1`, { waitUntil: 'networkidle' });
