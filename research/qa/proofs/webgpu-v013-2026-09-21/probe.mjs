...
const canonicalPath = '/works/webgpu-2026-09-21/';
const rawPath = '/studies/webgpu/v013/?preview=1&interaction=1';
const blindPath = '/studies/webgpu/v013/?preview=1&static=1&blind=1';
...
const frameSnapshot = async (frame) => frame.locator('body').evaluate(() => ({
  state: globalThis.__mutineState?.() ?? null,
...
async function runTableau(browser, path, viewport, reduced, kind) {
  ...
  await page.waitForTimeout(reduced ? 100 : 70);
...
try {
  ...
  const journal = await runRoute(browser, '/journal/', '#journal-webgpu-2026-09-21', 'journal-390x844.png');
