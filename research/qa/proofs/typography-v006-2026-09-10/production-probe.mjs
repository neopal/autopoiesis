...
const proofPath = 'research/qa/proofs/typography-v006-2026-09-10/production-results.json';
...
results.worksJson = await probe('/studio/data/works.json', async (page) => {
  ...
  const work = data.works.find((entry) => entry.id === 'typography-2026-09-10');
...
results.journal = await probe('/journal/', async (page) => {
  const anchor = page.locator('#journal-typography-2026-09-10');
...
results.canonical = await probe('/works/typography-2026-09-10/', async (page) => {
  ...
  return {
    ...
    tableauCanvas: await frame.locator('#piece').count(),
    topGeometry: await page.evaluate(() => ({ innerWidth, clientWidth: document.documentElement.clientWidth, scrollWidth: document.documentElement.scrollWidth })),
    frameGeometry: await frame.evaluate(() => ({ innerWidth, clientWidth: document.documentElement.clientWidth, scrollWidth: document.documentElement.scrollWidth, state: window.__mutineHandwritingV006?.getState() }))
  };
});

results.rawPreview = await probe('/studies/handwriting/v006/?preview=1&interaction=1', async (page) => {
  await page.locator('#piece').waitFor({ state: 'visible' });
  const before = await page.evaluate(() => window.__mutineHandwritingV006.getState());
  await page.locator('#piece').click({ position: { x: 195, y: 290 } });
  const after = await page.evaluate(() => window.__mutineHandwritingV006.getState());
  return {
    ...
    before,
    after,
    geometry: await page.evaluate(() => ({ innerWidth, clientWidth: document.documentElement.clientWidth, scrollWidth: document.documentElement.scrollWidth }))
  };
}, 'no-preference');

await browser.close();
const pass = results.worksJson.status === 200
  && results.worksJson.record.id === 'typography-2026-09-10'
  && results.journal.status === 200
  && results.journal.renderedAnchorCount === 1
  && results.journal.renderedTitle === 'The gap writes through its neighbours.'
  && results.canonical.status === 200
  && results.canonical.renderedTitle === 'The gap writes through its neighbours.'
  ...
  && results.rawPreview.after.memoryCount === 1
...
console.log(JSON.stringify({ base, status: output.status, results }, null, 2));
