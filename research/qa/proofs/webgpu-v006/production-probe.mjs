...
const canonicalReadback = await page.evaluate(() => ({
  ...
  mount: document.querySelector('[data-catalog-work-detail="webgpu-2026-09-10"]') !== null
}));
...
if (!frame || !canonicalReadback.mount || canonicalReadback.workId !== 'webgpu-2026-09-10' || !canonicalReadback.tableauFirst) failures.push('production canonical shell');
...
const journalReadback = await journalPage.evaluate(() => {
  const entry = document.querySelector('#journal-webgpu-2026-09-10');
  return {
    innerWidth,
    clientWidth: document.documentElement.clientWidth,
    scrollWidth: document.documentElement.scrollWidth,
    anchor: Boolean(entry),
    titleCount: document.body.textContent.includes('The crowd keeps a second route.') ? 1 : 0,
...
if (!journalReadback.anchor || journalReadback.titleCount !== 1 || journalReadback.href !== '/works/webgpu-2026-09-10/#journal') failures.push('production Journal entry');
