...
const canonicalPath = '/works/typography-2026-09-14/';
const rawPath = '/studies/handwriting/v010/?preview=1&interaction=1';
const viewports = [
...
async function openCanonical(browser, viewport, reducedMotion) {
  ...
  const frame = page.frames().find((candidate) => candidate.url().includes('/studies/handwriting/v010/'));
  assert(frame, 'embedded v010 frame did not load');
  await frame.waitForFunction(() => Boolean(window.__mutineHandwritingV010));
  ...
  if (reducedMotion) assert(tableau.state.stage === 13 && tableau.state.memory.length === 6 && tableau.state.paused, 'reduced motion did not settle the final hinge frame');
...
async function openInteraction(browser) {
  ...
  await page.waitForFunction(() => Boolean(window.__mutineHandwritingV010));
  ...
  assert(initial.controls.length === 3 && initial.controls.every((control) => control.height >= 44) && initial.controls.find((control) => control.id === 'place-hinge')?.disabled === false && initial.controls.find((control) => control.id === 'release-sequence')?.disabled === false && initial.controls.find((control) => control.id === 'lift-hinge')?.disabled === true, 'raw controls below 44px or wrong initial disabled state');
  ...
  const afterPointer = await page.evaluate(() => ({ state: window.__mutineHandwritingV010.getState(), signature: document.querySelector('#piece').toDataURL() }));
  assert(afterPointer.state.memory.length === 1 && afterPointer.state.paused && afterPointer.signature !== initial.signature, 'pointer did not change the structural field');
  await canvas.focus();
  await page.keyboard.press('Enter');
  const afterKeyboard = await page.evaluate(() => ({ state: window.__mutineHandwritingV010.getState() }));
  assert(afterKeyboard.state.memory.length === 2, 'keyboard did not add a second hinge');
  await page.locator('#lift-hinge').click();
  const afterLift = await page.evaluate(() => ({ state: window.__mutineHandwritingV010.getState(), signature: document.querySelector('#piece').toDataURL() }));
  assert(afterLift.state.memory.length === 1, 'lift did not remove only the latest hinge');
  ...
  const afterRelease = await page.evaluate(() => ({ state: window.__mutineHandwritingV010.getState() }));
...
async function openStaticBlind(browser) {
  ...
  const path = '/studies/handwriting/v010/?preview=1&static=1&blind=1';
  ...
  assert(state.state.stage === 13 && state.state.memory.length === 6 && !state.furnitureVisible, 'static blind did not preserve the final field without furniture');
...
async function openJournal(browser) {
  ...
  const journal = await page.evaluate(() => {
    const entry = document.querySelector('#journal-typography-2026-09-14');
    return {
      innerWidth,
      clientWidth: document.documentElement.clientWidth,
      scrollWidth: document.documentElement.scrollWidth,
  ...
  assert(journal.anchorCount === 1 && journal.titlePresent && journal.href === '/works/typography-2026-09-14/#journal', 'Journal record did not render');
