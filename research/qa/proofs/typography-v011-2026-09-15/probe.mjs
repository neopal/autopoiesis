...
const canonicalPath = '/works/typography-2026-09-15/';
const rawPath = '/studies/handwriting/v011/?preview=1&interaction=1';
const viewports = [
...
const results = { base, canonicalPath, rawPath, viewportRuns: [], interaction: null, staticBlind: null, journal: null, issues: [] };
...
async function openCanonical(browser, viewport, reducedMotion) {
  ...
  const frame = page.frames().find((candidate) => candidate.url().includes('/studies/handwriting/v011/'));
  assert(frame, 'embedded v011 frame did not load');
  await frame.waitForFunction(() => Boolean(window.__mutineHandwritingV011));
  ...
  if (reducedMotion) assert(tableau.state.stage === 15 && tableau.state.memory.length === 7 && tableau.state.paused, 'reduced motion did not settle the final stutter frame');
...
async function openInteraction(browser) {
  ...
  await page.waitForFunction(() => Boolean(window.__mutineHandwritingV011));
  ...
  assert(initial.controls.length === 3 && initial.controls.every((control) => control.height >= 44) && initial.controls.find((control) => control.id === 'place-stutter')?.disabled === false && initial.controls.find((control) => control.id === 'release-sequence')?.disabled === false && initial.controls.find((control) => control.id === 'lift-stutter')?.disabled === true, 'raw controls below 44px or wrong initial disabled state');
  ...
  const afterPointer = await page.evaluate(() => ({ state: window.__mutineHandwritingV011.getState(), signature: document.querySelector('#piece').toDataURL() }));
  assert(afterPointer.state.memory.length === 1 && afterPointer.state.paused && afterPointer.state.stutteredRoutes >= 3 && afterPointer.signature !== initial.signature, 'pointer did not change the structural field');
  await canvas.focus();
  await page.keyboard.press('Enter');
  const afterKeyboard = await page.evaluate(() => ({ state: window.__mutineHandwritingV011.getState() }));
  assert(afterKeyboard.state.memory.length === 2 && afterKeyboard.state.stutteredRoutes >= 3, 'keyboard did not add a second stutter');
  await page.locator('#lift-stutter').click();
  const afterLift = await page.evaluate(() => ({ state: window.__mutineHandwritingV011.getState(), signature: document.querySelector('#piece').toDataURL() }));
  assert(afterLift.state.memory.length === 1, 'lift did not remove only the latest stutter');
  ...
  const afterRelease = await page.evaluate(() => ({ state: window.__mutineHandwritingV011.getState() }));
...
async function openStaticBlind(browser) {
  ...
  const path = '/studies/handwriting/v011/?preview=1&static=1&blind=1';
  ...
  assert(state.state.stage === 15 && state.state.memory.length === 7 && !state.furnitureVisible, 'static blind did not preserve the final field without furniture');
...
async function openJournal(browser) {
  ...
  const journal = await page.evaluate(() => {
    ...
    return {
      ...
      titlePresent: document.body.textContent.includes('The sentence keeps a stutter.'),
      href: entry?.querySelector('h3 a')?.getAttribute('href') ?? null
    };
  });
  assert(journal.anchorCount === 1 && journal.titlePresent && journal.href === '/works/typography-2026-09-15/#journal', 'Journal record did not render');
...
try {
  ...
  results.issues = [
    ...results.viewportRuns,
    results.interaction,
    results.staticBlind,
    results.journal
  ].flatMap(issueMessages);
  ...
  process.stdout.write(JSON.stringify({ ok: true, viewportRuns: results.viewportRuns.length, issues: results.issues.length, interactionMemory: [results.interaction.initial.state.memory.length, results.interaction.afterPointer.state.memory.length, results.interaction.afterKeyboard.state.memory.length, results.interaction.afterLift.state.memory.length, results.interaction.afterRelease.state.memory.length], journalAnchors: results.journal.anchorCount, captures: 13 }, null, 2));
} finally {
  await browser.close();
}
